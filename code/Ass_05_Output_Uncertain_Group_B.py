...
Assignment 5: Analyze effects on model efficiency with Output Uncertainty
Group B
============================================== 

Tasks (from PDF):
1. Create 2000 perturbed observed discharge series
2. Compare CDFs of perturbed vs original 
...
Reference: 
Hydrological model calibration with uncertain discharge data, Westerberg et al.
2020, Paper

Towards a limits of acceptability approach to the calibration of hydrological
models - Extending observation error, Liu et al. 2009, Paper
...
# perturbation values to add: C = +/- 25 cm
lower_bd = -25
upper_bd = 25 # Results in ~5% std, up to ~25% at 3-sigma
...
def load_data():
    ...
    obs_wlvl = inp_dfe['ddho__ref'].values.astype(np.float32)
    ...
    return temps, precip, pet, obs_q, obs_wlvl, dslr, ref_params

def create_perturbed_series(obs_wlvl, n_series, lower_bd= -15.0, upper_bd=15, 
                            clip_multipliers=True, seed=None):
    '''
    Create n_series perturbed observed discharge series.
    fitting results (Q = a * h^b):
    First interval (100 <= h < 442): a = 1.5816e-05, b = 2.4815
    Second interval (h >= 442):       a = 6.6599e-39, b = 15.0966
    '''
    
    n_timesteps = len(obs_wlvl)
    
    # random number in interval: 
    multipliers = np.random.uniform(lower_bd,upper_bd,size=(n_series, n_timesteps))
    perturbed = obs_wlvl[np.newaxis, :] + multipliers
    
    # Keep all non-negative
    perturbed = np.maximum(perturbed, 0.0)
    
    '''using result from fitting3.3.2026.py: to calculate discharge ''' 
    
    perturbed_dis = perturbed.copy()
    
    for n in range(0,len(perturbed)):
        for m in range(0,n_timesteps):
            a = perturbed[n,m] - 315.640655
            if a < 0:
                b = abs(a)
                Q2 = 5.83762819e-06 * (-(b ** 3.3829))
            else:
                Q2 = 5.83762819e-06 * (a ** 3.3829)
            Q1 = 1.22871059e-04 * (perturbed[n,m] - 25.033145)**2.1538 
            #Q2 = 5.83762819e-06 * (perturbed[n,m] - 315.640655)**3.3829
            w  = 1 / (1 + math.exp(-(perturbed[n,m] - 450.0000) / 6.8602))
            perturbed_dis[n,m] = (1-w) * Q1 + w * Q2
          
    dis_cal = obs_wlvl.copy()
    for n in range(0,len(obs_wlvl)):
        a = obs_wlvl[n] - 315.640655
        if a < 0:
            b = abs(a)
            Q2 = 5.83762819e-06 * (-(b ** 3.3829))
        else:
            Q2 = 5.83762819e-06 * (a ** 3.3829)
        Q1 = 1.22871059e-04 * (obs_wlvl[n] - 25.033145)**2.1538  
        w  = 1 / (1 + math.exp(-(obs_wlvl[n] - 450.0000) / 6.8602))
        dis_cal[n] = (1-w) * Q1 + w * Q2
    
    return perturbed_dis.astype(np.float32), dis_cal.astype(np.float32)
...
def run_diagnostic_convergence_test(k_series, perturbed_ppt, temps, pet, obs_q, 
    ...
    results = []
    for idx in enumerate(test_indices):
...
def plot_ppt_cdf_comparison(original, perturbed_all, out_dir):
    """
    Plot CDF comparison of original vs perturbed discharge.
    ...
    ax.set_xlabel('Discharge [m^3*s^(-1)]', fontsize=11)
    ax.set_ylabel('Cumulative Probability', fontsize=11)
    ax.set_title('CDF Comparison: Original vs Perturbed Discharge\n'
    ...
    plt.tight_layout()
    fig.savefig(out_dir / 'dis_cdf_comparison.png', dpi=150)
    plt.close(fig)
    print("  Saved: dis_cdf_comparison.png")
...
def plot_ppt_cdf_comparison_two(original, perturbed_all, out_dir):
    ...
    perturbed_dis = np.stack([np.cumsum(n) for n in perturbed_all])
    
    cdf_orig = np.cumsum(original)
    tsp = range(0,len(original))
    
    for n in perturbed_dis:
    ...
    ax.set_ylabel('Cumulative Discharge', fontsize=11)
    ax.set_title('Comparison: Original vs Perturbed Discharge\n'
                 f'(Envelope from {len(perturbed_all)} perturbed series)',
                 fontsize=12)
    ax.legend(loc='lower right')
    ax.grid(True, alpha=0.3)
    ...
    plt.tight_layout()
    fig.savefig(out_dir / 'dis_cdf_comparison_two.png', dpi=150)
    plt.close(fig)
    print("  Saved: dis_cdf_comparison_two.png")

def plot_ppt_cdf_comparison_sort(original, perturbed_all, out_dir):

    fig, ax = plt.subplots(figsize=(10, 6), dpi=150)
        
    perturbed_dis = np.stack([np.cumsum(np.sort(n)) for n in perturbed_all])
    
    cdf_orig_0 = np.sort(original)
    cdf_orig = np.cumsum(cdf_orig_0)
    tsp = range(0,len(original))
    ...
    plt.tight_layout()
    fig.savefig(out_dir / 'dis_comparison_sort.png', dpi=150)
    plt.close(fig)
    print("  Saved: dis_comparison_sort.png")
...
def plot_ofv_cdfs(ref_ofv, ofv_reference_params, ofv_recalibrated, out_dir, 
    ...
    ax.set_title('OFV Distribution: Reference Params vs Recalibrated\n'
                 'Using Perturbed Discharge', fontsize=12)
    ax.legend(loc='lower right')
    ax.grid(True, alpha=0.3)
...
def plot_scatter_relative_change_vs_ofv(original_ppt, perturbed_all, 
    ...
    ax1.set_xlabel('Mean Absolute Relative DIS Change [%]', fontsize=11)
    ...
    if has_recalibration and ofv_recalibrated is not None and len(axes) > 1:
        ...
        ax2.set_xlabel('Mean Absolute Relative DIS Change [%]', fontsize=11)
...
def save_results_summary(ref_params, ref_ofv, cal_nse, ofv_reference, ofv_recalibrated,
    ...
    with open(out_dir / 'uncertainty_analysis_summary.txt', 'w') as f:
        f.write("Assignment 5: Model Output Uncertainty Analysis\n")
        f.write("=" * 70 + "\n\n")
        
        f.write("Configuration:\n")
        f.write(f"  Number of perturbed series: {args.n}\n")
        f.write(f"  Perturbation Interval for water level: [{lower_bd}, {upper_bd}]\n")
        ...
        f.write(f"  calculated NSE: {cal_nse:.6f}\n")
        ...
        f.write("Mean Absolute Relative DIS Change:\n")
        ...
        if has_recalibration and ofv_recalibrated is not None:
            ...
            f.write("\n  Conclusion:\n")
            f.write("  - Output uncertainty causes significant parameter uncertainty.\n")
            f.write("  - Accounting for Output uncertainty during calibration could:\n")
...
Examples:
  ...
  python %(prog)s --n 2000 --recalibrate --no-clip-multipliers
    ...
    parser.add_argument('--maxiter', type=int, default=600,
                        help='DE max iterations for recalibration (default: 600)')
...
def main():
    ...
    print("\n" + "=" * 70, flush=True)
    print("ASSIGNMENT 5: MODEL OUTPUT UNCERTAINTY ANALYSIS", flush=True)
    print("=" * 70, flush=True)
    ...
    out_dir = Path(__file__).resolve().parent / 'outputs' / 'assignment5'
    ...
    temps, precip, pet, obs_q, obs_wlvl, dslr, ref_params = load_data()
    n_steps = len(temps)
    ...
    # Create perturbed discharge series
    print(f"\n[PERTURBATION GENERATION]", flush=True)
    print(f"  Creating {args.n} perturbed observed discharge series...", flush=True)
    print(f"  Perturbation Interval: [{lower_bd}, {upper_bd}]", flush=True)
    clip_mult = args.clip_multipliers
    
    perturbed_dis, dis_cal = create_perturbed_series(
        obs_wlvl, args.n, lower_bd, upper_bd, 
        clip_multipliers=clip_mult, seed=args.seed
    )
    print(f"  Done. Shape: {perturbed_dis.shape}", flush=True)
    
    cal_sim = run_model_with_params(ref_params, temps, precip, pet, dslr, n_steps, verbose=True)
    cal_nse = compute_nse(dis_cal, cal_sim)
    print(f'NSE with calculated function: {cal_nse}')
    diff = ref_nse - cal_nse
    print(f'differnece to ref_sim: {diff}')
    ...
    inp_dfe = pd.read_csv(data_dir / 'time_series___24163005.csv', 
                          sep=';', index_col=0)
    inp_dfe.index = pd.to_datetime(inp_dfe.index, format='%Y-%m-%d-%H')
    print(perturbed_dis[0])
    print(perturbed_dis[1999])
    plot_diss(inp_dfe, obs_q, perturbed_dis[0], out_dir, -25)
    plot_diss(inp_dfe, obs_q, perturbed_dis[1999], out_dir, +25)
    cal_nse_neg25 = compute_nse(dis_cal, perturbed_dis[1999])
    print(f'NSE_cal_neg25: {cal_nse_neg25}')
    
    # Plot PPT CDF comparison
    print("\n[PPT CDF VERIFICATION]", flush=True)
    plot_ppt_cdf_comparison(obs_q, perturbed_dis, out_dir)
    plot_ppt_cdf_comparison_two(obs_q, perturbed_dis, out_dir)
    plot_ppt_cdf_comparison_sort(obs_q, perturbed_dis, out_dir)
    ...
    if n_to_eval > 0:
        ...
        results_ref = Parallel(n_jobs=args.jobs, verbose=1)(
            delayed(evaluate_single_series_reference)(
                i, precip, temps, pet, perturbed_dis[i], dslr, ref_params
    ...
    if args.diagnostic_k > 0:
        run_diagnostic_convergence_test(
            args.diagnostic_k, precip, temps, pet, perturbed_dis, 
            dslr, ref_params, args, out_dir
        )
            ...
            for batch_start in range(0, n_to_process, batch_size):
                ...
                results_batch = Parallel(n_jobs=args.jobs, verbose=10)(
                    delayed(calibrate_single_series)(
                        i, precip, temps, pet, perturbed_dis[i], dslr,
                ...
                if completed < args.n:
                    ...
                    if len(completed_ids) > 0:
                        marc_completed = [compute_mean_abs_relative_change(obs_q, perturbed_dis[i]) for i in completed_ids]
                        save_checkpoint(
    ...
    marc_values = plot_scatter_relative_change_vs_ofv(
        obs_q, perturbed_dis, ofv_reference, ofv_recalibrated, ref_ofv, out_dir,
        has_recalibration=args.recalibrate
    )
    ...
    save_results_summary(ref_params, ref_ofv, cal_nse, ofv_reference, ofv_recalibrated,
                          recalib_params_all, marc_values, out_dir, args,
                          has_recalibration=args.recalibrate, 
    ...
    # plt.boxplot(recalib_params_all, labels=PARAM_NAMES)
        
    fig, ax = plt.subplots(figsize=(12, 6)) 
    ax.boxplot(recalib_params_all) 
    ax.set_xticklabels([f'{PARAM_NAMES[i]}' for i in range(18)], rotation=45, ha='right') 
    ax.set_ylabel('Values') 
    fig.savefig(out_dir / 'parm_boxplot.png', dpi=150, 
                bbox_inches='tight')
    plt.close(fig)
