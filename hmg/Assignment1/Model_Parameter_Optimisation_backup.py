...
script_dir = Path(__file__).resolve().parent     # hmg/Assignment1/
...
class DERecorder:
    ...
    def _assemble_full_params(self, prms_opt):
        ...
        """
        full = np.zeros((PARAM_NAMES_OPT,), dtype=np.float32)
        # place fixed first
        for idx, val in self.fixed_params.items():
            full[idx] = np.float32(val)
        # then place optimized entries in order
        prms_opt = np.asarray(prms_opt, dtype=np.float32)
    ...
    def __call__(self, prms):
        ...
        if self.eval_count == self.pop_sz*len(prms) or self.eval_count == self.pop_sz*len(prms)*self.gen_cnt:
            ...
            self.gen_best_objs.append(float(dfe_min[0].iloc[0]))
            self.gen_max_objs.append(float(max(dfe[0])))
            self.gen_med_obj.append(float(np.median(dfe[0])))
            self.gen_best_params.append(dfe_min[1].iloc[0])
...
def plot_param_evolution_per_generation(pop_sz, run_min, eval_params, gen_best_parms, best_params_opt, out_file, param_names):
    """Plot how the best parameter values evolve across generations.
    
    Args:
        pop_sz: Population size (unused but kept for API compatibility)
        run_min: Running minimum objective per generation (unused)
        eval_params: List of all evaluated parameter sets (unused)
        gen_best_parms: List of best parameter set per generation (main data)
        best_params_opt: Final best parameter set (unused)
        out_file: Output file path
        param_names: List of parameter names
    """
    if len(gen_best_parms) == 0:
        return
    
    # Stack best-per-generation parameters into 2D array: (n_generations, n_params)
    gen_best = np.vstack(gen_best_parms)
    n_gens = gen_best.shape[0]
    n_params = gen_best.shape[1]
    
    generations = np.arange(1, n_gens + 1)
    
    fig, axs = plt.subplots(n_params, 1, figsize=(10, 2*n_params), dpi=120, sharex=True)
    if n_params == 1:
        axs = [axs]
    
    for i in range(n_params):
        axs[i].plot(generations, gen_best[:, i], linewidth=1.2, marker='o', markersize=2)
        axs[i].set_ylabel(param_names[i], fontsize=9)
        axs[i].grid(True, alpha=0.3)
    
    axs[-1].set_xlabel('Generation')
    fig.suptitle('Best Parameter Values per Generation', fontsize=12)
...
def plot_param_scatter_all_evals(eval_params, eval_objs, bounds, out_dir, param_names):
    ...
    # No filtering - show all evaluations including poor ones (objective > 1 means NSE < 0)
    ...
    for i in range(n_params):
        ...
        # X: raw parameter values; Y: objective value
        ax.scatter(eval_params[:, i], eval_objs, s=10, alpha=0.6)
...
def plot_convergence_per_generation(gen_max_obj, gen_best_obj, gen_med_obj, out_dir):
    ...
    #   return
    #mins, maxs, meds = [], [], []
    #for (s, e) in gen_bounds:
        #if e <= s:
            #mins.append(np.nan); maxs.append(np.nan); meds.append(np.nan)
        #else:
            #vals = np.array(eval_objs[s:e])
            #mins.append(np.nanmin(vals))
            #maxs.append(np.nanmax(vals))
            #meds.append(np.nanmedian(vals))
    mins = np.array(gen_best_obj); maxs = np.array(gen_max_obj); meds = np.array(gen_med_obj)
    ...
    ax.legend()
    ax.grid(True)
    # No y-axis clipping - show full distribution as required
    ...
    ax.set_xlabel('Generation')
    ax.set_ylabel('Objective (1 - NSE)')
    ax.set_title('Convergence per generation (zoomed)')
    ax.legend()
    ax.grid(True)
    # Zoomed view for detail - limit to reasonable range
    ax.set_ylim(0.0, 1.5)
    ...
    ax.legend()
    ax.grid(True)
    # lock y-range to [0, 50] explicitly
    #ax.set_ylim(0.0, 3.0)
...
def find_data_dir():
    """
    Find the 'data' directory using project_root (defined at module level).
...
def main():
    ...
    # FAST_DEBUG mode: set to True for quick testing (5 generations)
    FAST_DEBUG = False
    if FAST_DEBUG:
        gen = 5
        pop_sz = 5
    ...
    best_params_opt = de_result.x.astype(np.float32)
    print('')
    ...
    if len(recorder.eval_params) > 0: # gen_best_params
        plot_param_evolution_per_generation(pop_sz, run_min, recorder.eval_params,recorder.gen_best_params, best_params_opt,
                                            out_file=str(out_dir / 'params_evolution_per_generation.png'),
                                            param_names=PARAM_NAMES_OPT)
