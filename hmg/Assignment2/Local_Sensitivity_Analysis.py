...
script_dir = Path(__file__).resolve().parent     # hmg/Assignment2/
...
def main():
    ...
    # Optimized parameters from Assignment 1 (NSE = 0.834)
    # These are the calibrated parameters used as reference for sensitivity analysis
    prms = np.array([
        0.00,  # 'snw_dth'
        -0.00616,  # 'snw_att'
        0.10434,  # 'snw_pmf'
    ...
    with open(out_dir/'summary_outside_boundaries.txt', 'w') as txt_hdl:
        txt_hdl.write(f'\n')
    for i in range(0, len(prms)): 
        with open(out_dir/'summary_outside_boundaries.txt', 'a') as txt_hdl:
    ...
    for i in range(0, len(X)):
        ...
        if i == j*120 + 0:
            print('-30')
            out_dir_plots = act_dir / 'results' / 'minus_30_percent'
            ...
            plot_diss(inp_dfe, diso, diss, out_dir_plots, str(f'{PARAM_NAMES_OPT[j]}_minus_30_percent'))
            plot_input(inp_dfe, otps, otps_lbls, out_dir_plots, str(f'{PARAM_NAMES_OPT[j]}_minus_30_percent'))

        elif i == j*120 + 19: 
            print('-20')
            out_dir_plots = act_dir / 'results' / 'minus_20_percent'
            out_dir_plots.mkdir(parents=True, exist_ok=True)
            ...
            print('-10')
            out_dir = act_dir / 'plot' / 'SA' / 'minus_10_percent'
            out_dir.mkdir(parents=True, exist_ok=True)
            ...
            print('+10')
            out_dir = act_dir / 'plot' / 'SA' / 'plus_10_percent'
            out_dir.mkdir(parents=True, exist_ok=True)
            ...
            print('+20')
            out_dir = act_dir / 'plot' / 'SA' / 'plus_20_percent'
            out_dir.mkdir(parents=True, exist_ok=True)
            ...
            print('+30')
            out_dir_plots = act_dir / 'results' / 'plus_30_percent'
            out_dir_plots.mkdir(parents=True, exist_ok=True)
            ...
            plot_diss(inp_dfe, diso, diss, out_dir_plots, str(f'{PARAM_NAMES_OPT[j]}_plus_30_percent'))
            plot_input(inp_dfe, otps, otps_lbls, out_dir_plots, str(f'{PARAM_NAMES_OPT[j]}_plus_30_percent'))
                
        prms_lpv  = []
    #==========================================================================
    out_dir_final = act_dir / 'results'
    out_dir_final.mkdir(parents=True, exist_ok=True)
    
   
    for i in range(0, len(prms)):
        j = i * 120  # Start index for this parameter's Y values
        if i != 1:  # Not snw_att (temperature parameter)        
            plot_param_scatter(x, Y[j : (j + 120)], out_dir_final, PARAM_NAMES_OPT[i])
        else:
            plot_param_scatter_T(x_T, Y[j : (j + 120)], out_dir_final, PARAM_NAMES_OPT[i])
